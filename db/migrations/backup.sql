...
\restrict fdIa3Ld5pQ4SjFKRvXMfJahLO7zZkLsNA9McYT1uQAd3wRQUfUDkqpOq3ULh44a
...
CREATE TABLE public.subjects (
    ...
    sync_subject boolean DEFAULT false NOT NULL
);
...
14	Assignment 1	5	10.3	0.8583	12	numeric	f
15	Assignment 2	5	11.04	0.92	12	numeric	f
16	Assignment 3	5	12	0.75	16	numeric	f
17	Mid-Term Quiz	4	13.2	0.66	20	numeric	f
18	Assignment 1	4	9.5	0.95	10	numeric	f
19	Assignment 2	4	9.5	0.95	10	numeric	f
20	Assignment 3	4	9.5	0.95	10	numeric	f
...
26	Week 6 Quiz	7	4.23	0.846	5	numeric	f
...
43	Discrete Exam	11	24	0.8	30	numeric	f
...
71	Quiz 1	18	4	0.8	5	numeric	f
...
60	Lab Exercises	15	8	1	8	numeric	f
59	Assessment 3	15	10	1	10	numeric	f
61	Initial Project Description	16	\N	\N	\N	S	f
72	Quiz 2	18	3	0.6	5	numeric	f
62	Design Progress	16	10.8	0.72	15	numeric	f
63	Project Requirements and Interface Presentation	16	7.5	0.75	10	numeric	f
64	Prototype Presentation	16	12.15	0.81	15	numeric	f
66	Marketing & Technical Presentation	16	12.15	0.81	15	numeric	f
65	Final Product & Documentation	16	36.45	0.81	45	numeric	t
67	Quiz 1	17	4.5	0.9	5	numeric	f
68	Quiz 2	17	9.5	0.95	10	numeric	f
69	Assignment	17	11.3	0.7533	15	numeric	f
70	Group Project	17	18.6	0.744	25	numeric	f
73	Assignment	18	14.5	0.9667	15	numeric	f
74	Group Project - Project Presentation	18	4.5	0.72	6.25	numeric	f
75	Group Project - Project Report	18	13.5	0.72	18.75	numeric	f
76	Lab Test 1	19	3.5	0.7	5	numeric	f
77	Lab Test 2	19	5.5	0.55	10	numeric	f
78	Assignment 1	19	14	0.9333	15	numeric	f
79	Assignment 2	19	13	0.65	20	numeric	f
80	Lab Assessment 1	20	1.5	0.25	6	numeric	f
81	Lab Assessment 2	20	5.08	0.7257	7	numeric	f
82	Lab Assessment 3	20	7	1	7	numeric	f
83	Assignment	20	30	1	30	numeric	f
84	Assignment 1	21	9.8	0.98	10	numeric	f
85	Assignment 2	21	16.4	0.82	20	numeric	f
86	Assignment 3	21	19.5	0.975	20	numeric	f
\.
...
COPY public.courses (id, name, code, grading_scale) FROM stdin;
1	Bachelor of Computer Science	766	6
\.
...
COPY public.examinations (id, subject_id, exam_mark, exam_weight) FROM stdin;
1	1	56.145	60
2	2	42.525	50
3	3	38.9	60
4	5	45.66	60
...
1	Standard	HD	High Distinction	85	4	both
2	Standard	D	Distinction	75	3.7	both
3	Standard	C	Credit	65	3.3	both
4	Standard	P	Pass	50.01	2	both
5	Standard	PS	Pass Supplementary	50	2	both
6	Standard	F	Fail	0	0	both
\.
...
COPY public.subjects (id, subject_code, semester_id, subject_name, total_mark, credit_points, sync_subject) FROM stdin;
16	CSIT321	5	Capstone Project	79	12	t
1	CSIT110	1	Fundamental Programming with Python	95	6	f
2	CSIT114	1	System Analysis	80	6	f
3	CSIT123	1	Computing and Cyber Security Fundamentals	73	6	f
5	CSIT115	2	Data Management and Security	79	6	f
4	CSIT121	2	Object Oriented Design and Programming	80	6	f
6	CSIT128	2	Introduction to Web Technology	93	6	f
7	CSIT226	2	Human Computer Interaction	69	6	f
8	CSCI235	3	Database Systems	79	6	f
9	CSIT214	3	IT Project Management	68	6	f
10	CSIT302	3	Cybersecurity	93	6	f
11	MATH255	3	Mathematics for Computing	67	6	f
12	CSCI203	4	Algorithms and Data Structures	66	6	f
13	CSCI218	4	Foundations of Artificial Intelligence	75	6	f
14	CSCI251	4	Advanced Programming	68	6	f
15	CSCI262	4	System Security	68	6	f
17	CSCI316	6	Big Data Mining Techniques & Implementation	62	6	f
18	CSCI323	6	Modern Artificial Intelligence	50	6	f
19	CSIT375	6	Artificial Intelligence for Cybersecurity	50	6	f
20	CSCI369	7	Ethical Hacking	82	6	f
21	ISIT312	7	Big Data Management	85	6	f
\.
...
SELECT pg_catalog.setval('public.assignments_id_seq', 87, true);
...
SELECT pg_catalog.setval('public.exam_settings_id_seq', 22, true);
...
SELECT pg_catalog.setval('public.examinations_id_seq', 23, true);
...
SELECT pg_catalog.setval('public.grade_scales_id_seq', 6, true);
...
--
-- Name: ix_examinations_subject_id; Type: INDEX; Schema: public; Owner: Michael
--

CREATE UNIQUE INDEX ix_examinations_subject_id ON public.examinations USING btree (subject_id);
...
ALTER TABLE ONLY public.exam_settings
    ADD CONSTRAINT exam_settings_subject_id_fkey FOREIGN KEY (subject_id) REFERENCES public.subjects(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.examinations
    ADD CONSTRAINT examinations_subject_id_fkey FOREIGN KEY (subject_id) REFERENCES public.subjects(id) ON DELETE CASCADE;


--
-- Name: courses fk_courses_grading_scale; Type: FK CONSTRAINT; Schema: public; Owner: Michael
--

ALTER TABLE ONLY public.courses
    ADD CONSTRAINT fk_courses_grading_scale FOREIGN KEY (grading_scale) REFERENCES public.grade_scales(id);
...
-- Add has_exam column after subjects table creation
ALTER TABLE public.subjects ADD COLUMN has_exam BOOLEAN DEFAULT false NOT NULL;

-- Set has_exam for subjects under course 766 after subjects data is loaded
UPDATE public.subjects
SET has_exam = true
WHERE semester_id IN (
    SELECT id FROM public.semesters WHERE course_id = (
        SELECT id FROM public.courses WHERE code = '766'
    )
);
