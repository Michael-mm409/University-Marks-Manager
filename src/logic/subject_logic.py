def add_subject(app):
    ...
    subejct_code = app.simpledialog.askstring("Add Subject", "Enter the subject code:")
    subject_name = app.simpledialog.askstring("Add Subject", "Enter the subject name:")
    sync_source = app.sync_source_var.get()  # Reads the checkbox values

    if subejct_code and subject_name:
        semester_name = app.sheet_var.get()
        app.semesters[semester_name].add_subject(subejct_code, subject_name, sync_source)
        app.update_treeview()

def remove_subject(app):
    """Removes the selected subject from the selected semester."""
    subject_code = app.simpledialog.askstring("Remove Subject", "Enter the subject code to remove:")
    if subject_code:
        semester_name = app.sheet_var.get()
