...
def create_form_frame(
    ...
    self.semester_menu = ttk.OptionMenu(
        form_frame, sheet_var, sheet_var.get(), *sorted(semesters.keys()), command=update_semester
    )
    ...
    year_menu = ttk.OptionMenu(form_frame, year_var, year_var.get(), *year_list, command=update_year)
